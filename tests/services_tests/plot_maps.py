...
def main():
    ...
    for data in maps["data"]:
        ...
        for i, piece in enumerate(data['Player1Pieces']):
            ax.plot(piece['Position']['x'], piece['Position']['y'], 
                    'bo', markersize=8)
            plt.text(piece['Position']['x'], piece['Position']['y'] + 0.005, f'P{i}', ha='center')

        # Plot player 2 pieces (red circles)
        for i, piece in enumerate(data['Player2Pieces']):
            ax.plot(piece['Position']['x'], piece['Position']['y'], 
                    'ro', markersize=8)
            plt.text(piece['Position']['x'], piece['Position']['y'] + 0.005, f'P{i}', ha='center')
